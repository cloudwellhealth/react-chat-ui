...
const Buffer = styled.div<{ heightWithPx: string }>`
  height: ${({ heightWithPx }) => heightWithPx};
  width: 100%;
  position: relative;
`;
...
export default function MessageList({
  ...
  const { detectBottom, detectTop } =
    useDetectScrollPosition(scrollContainerRef);
        ...
        {loading ? (
          ...
          <>
            ...
            >
              {/* top buffer */}
              <div>
                <Buffer heightWithPx={showHeader ? '60px' : '0px'} />
              </div>
              ...
              {messages &&
                scrollContainerRef.current &&
                bottomBufferRef.current &&
                messages.map(({ user, text, image }, index) => {
                  ...
                  if (index > 0 && messages[index - 1].user.id !== user.id) {
                    firstClusterMessage = true;
                  }
                  ...
                  if (
                    ...
                    messages[index + 1].user.id !== user.id
                  ) {
                    lastClusterMessage = true;
                    last = true;
                  }
                  ...
                  if (
                    ...
                    return (
                      <Message
                        ...
                        loading={
                          index === messages.length - 1 && sendMessageLoading
                        }
              ...
              {/* bottom buffer */}
              <div>
                <Buffer heightWithPx="60px" ref={bottomBufferRef} />
              </div>
            </ScrollContainer>
          </>
        )}
